...
<section class="mx-auto max-w-[1280px] px-4 pb-8 pt-10 sm:px-6 lg:px-8" data-motion-section>
    <div class="overflow-hidden rounded-[36px] bg-white px-6 py-8 shadow-[0_20px_60px_rgba(15,23,42,0.08)] ring-1 ring-black/5 sm:px-8 lg:px-10 lg:py-10">
        <div class="grid gap-8 lg:grid-cols-[minmax(0,1.5fr)_minmax(19rem,0.92fr)] lg:items-end">
            <div class="max-w-3xl">
                <p class="text-sm font-medium uppercase tracking-[0.22em] text-zinc-400" data-motion-kicker>Categories Directory</p>
...
<section class="mx-auto max-w-[1280px] px-4 pb-20 sm:px-6 lg:px-8" data-motion-section>
    <div class="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
            <p class="text-sm font-medium uppercase tracking-[0.2em] text-zinc-400" data-motion-kicker>All Categories</p>
            <h2 class="mt-3 text-[2.25rem] font-semibold tracking-[-0.04em] text-zinc-950" data-motion-title>Browse all categories and their services</h2>
                ...
                <div class="grid gap-0 md:grid-cols-[15rem_minmax(0,1fr)]">
                    ...
                    <div class="space-y-6 px-6 py-6">
                        ...
                        <div class="flex flex-wrap gap-3">
                            <a href="{{ route('site.services.index', ['category' => $category->slug]) }}" class="inline-flex min-w-[160px] items-center justify-center rounded-[12px] bg-zinc-950 px-4 py-3.5 text-sm font-medium text-white transition hover:bg-zinc-800" data-motion-action>
                                Explore Services
                            </a>
                            <a href="{{ route('site.booking') }}" class="inline-flex min-w-[160px] items-center justify-center rounded-[12px] border border-zinc-300 px-4 py-3.5 text-sm font-medium text-zinc-700 transition hover:border-zinc-950 hover:text-zinc-950" data-motion-action>
                                Start Booking
                            </a>
                        </div>
                    </div>
        ...
        @empty
            <div class="col-span-full rounded-[32px] border border-dashed border-zinc-300 bg-white px-8 py-14 text-center shadow-[0_18px_50px_rgba(15,23,42,0.04)]" data-motion-card>
                ...
                <div class="mt-6 flex flex-wrap justify-center gap-3">
                    <a href="{{ route('site.categories.index') }}" class="inline-flex min-w-[160px] items-center justify-center rounded-[14px] bg-zinc-950 px-5 py-3.5 text-sm font-medium text-white transition hover:bg-zinc-800" data-motion-action>
                        Reset Filters
                    </a>
...
<div class="pointer-events-none fixed inset-0 z-50 bg-black/35 opacity-0 transition duration-300" data-filter-overlay="categories"></div>

<aside id="categoryFilterDrawer" class="fixed inset-y-0 right-0 z-50 flex w-full max-w-md translate-x-full flex-col bg-white shadow-2xl transition duration-300" data-filter-drawer="categories">
    <div class="flex items-center justify-between border-b border-black/5 px-6 py-5">
        ...
        <button type="button" class="inline-flex h-11 w-11 items-center justify-center rounded-full border border-zinc-200 text-zinc-700 transition hover:border-zinc-950 hover:text-zinc-950" aria-label="Close category filters" data-motion-action data-filter-close="categories">
    ...
    <form method="GET" action="{{ route('site.categories.index') }}" class="flex flex-1 flex-col overflow-y-auto px-6 py-6">
        ...
        <div class="mt-8 flex flex-wrap gap-3 border-t border-black/5 pt-6">
            ...
            <a href="{{ route('site.categories.index') }}" class="inline-flex min-w-[150px] items-center justify-center rounded-[14px] border border-zinc-300 px-5 py-3.5 text-sm font-medium text-zinc-700 transition hover:border-zinc-950 hover:text-zinc-950" data-motion-action>
                Reset
            </a>
        </div>
    </form>
